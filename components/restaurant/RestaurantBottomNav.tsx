import React from 'react';
import { useApp } from '../../context/AppContext';
import { RestaurantTab } from '../../types';
import { LayoutDashboard, ClipboardList, UtensilsCrossed, User } from 'lucide-react';

export const RestaurantBottomNav: React.FC = () => {
  const { restaurantTab, setRestaurantTab, orders, currentUser } = useApp();

  const myRestaurantId = currentUser.restaurantId || 'rest-boom-pizza';
  const pendingCount = orders.filter(o => o.restaurantId === myRestaurantId && o.status === 'PLACED').length;

  const tabs: { id: RestaurantTab; label: string; icon: React.ElementType; badge?: number }[] = [
    { id: 'DASHBOARD', label: 'داشبۆرد', icon: LayoutDashboard },
    { id: 'ORDERS', label: 'ئۆردەرەکان', icon: ClipboardList, badge: pendingCount },
    { id: 'MENU', label: 'مێنیو', icon: UtensilsCrossed },
    { id: 'ACCOUNT', label: 'هەژمار', icon: User }
  ];

  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-100 shadow-lg" dir="rtl">
      <div className="max-w-md mx-auto px-4 py-1.5 flex items-center justify-around">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          const isActive = restaurantTab === tab.id;

          return (
            <button
              key={tab.id}
              onClick={() => setRestaurantTab(tab.id)}
              className={`flex flex-col items-center justify-center py-1 px-3 rounded-2xl transition-all relative ${
                isActive
                  ? 'text-blue-600 font-black'
                  : 'text-slate-400 hover:text-slate-600 font-medium'
              }`}
            >
              <div className="relative">
                <Icon size={20} className={isActive ? 'stroke-[2.5]' : 'stroke-[1.8]'} />
                {tab.badge !== undefined && tab.badge > 0 && (
                  <span className="absolute -top-1.5 -right-2 bg-blue-600 text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center shadow-xs">
                    {tab.badge}
                  </span>
                )}
              </div>
              <span className="text-[10px] mt-0.5 tracking-tight">{tab.label}</span>
              {isActive && (
                <div className="w-1 h-1 bg-blue-600 rounded-full mt-0.5" />
              )}
            </button>
          );
        })}
      </div>
    </nav>
  );
};
